...
local function emit_live_state(force)
    ...
    if not force and signature == last_live_state_signature then return end
...
function send_slots(slots_info, generation, selected_game, selected_subgame)
    ...
    if contains_party then pcall(function() emit_live_state(true) end) end
    return success
end
...
local function poll_live_battle_state()
    ...
    if frame - last_live_poll_frame < live_poll_frame_interval then return end
