-- Local video bridge for DeSmuME.
-- The upper DS screen is captured in DeSmuME's native GD format and transported
-- at native resolution. Only the incoming picture-in-picture is reduced to 64x48
...
local function capture_upper_screen(frame)
    ...
    if type(gui.gdscreenshot) ~= "function" then
        if not capture_warning_printed then
            print("[Stream] gui.gdscreenshot ist in dieser DeSmuME-Version nicht verfügbar.")
    ...
    if not ok or not valid_gd_frame(data) then
        ...
        if capture_failure_count == 30 then
            print("[Stream] Warnung: 30 Screenshot-Frames konnten nacheinander nicht gelesen werden.")
        end
        return
    end
...
local function draw_incoming_frame()
    ...
    if type(gui.gdoverlay) ~= "function" then
        if not overlay_warning_printed then
            print("[Stream] gui.gdoverlay ist in dieser DeSmuME-Version nicht verfügbar.")
...
elseif emu ~= nil and type(emu.registerafter) == "function" then
    ...
    print("[Stream] Kein Frame-Callback verfügbar; Video-Streaming deaktiviert.")
    return false
end
