...
dofile "write_soulbuddy_snapshot.lua"
...
function getGameName()
    ...
    if game < 3 then return gameNames[game][subgame] end
...
function getCurFoeHP()
    if game == 1 then
        if mode == 4 then return memory.readword(pointer + 0x5574C)
        elseif mode == 3 then return memory.readword(pointer + 0x5580C)
        else return memory.readword(pointer + 0x5568C) end
    elseif game == 2 then
        if mode == 4 then return memory.readword(pointer + 0x56FC0)
        elseif mode == 3 then return memory.readword(pointer + 0x57080)
        else return memory.readword(pointer + 0x56F00) end
    else
        if mode == 4 then return memory.readword(pointer + 0x54764)
        elseif mode == 3 then return memory.readword(pointer + 0x54824)
        else return memory.readword(pointer + 0x546A4) end
    end
end
...
function getPidAddr()
    if game == 1 then
        ...
        if mode == 5 then return pointer + 0x36C6C
        elseif mode == 4 then return memory.readdword(enemyAddr) + 0x774 + 0x5B0 + 0xEC*(submode-1)
        elseif mode == 3 then return memory.readdword(enemyAddr) + 0x774 + 0xB60 + 0xEC*(submode-1)
        elseif mode == 2 then return memory.readdword(enemyAddr) + 0x774 + 0xEC*(submode-1)
        else return pointer + 0xD2AC + 0xEC*(submode-1) end
    elseif game == 2 then
        enemyAddr = pointer + 0x37970
        if mode == 5 then return pointer + 0x38540
        elseif mode == 4 then return memory.readdword(enemyAddr) + 0x1C70 + 0xA1C + 0xEC*(submode-1)
        elseif mode == 3 then return memory.readdword(enemyAddr) + 0x1C70 + 0x1438 + 0xEC*(submode-1)
        elseif mode == 2 then return memory.readdword(enemyAddr) + 0x1C70 + 0xEC*(submode-1)
        else return pointer + 0xD088 + 0xEC*(submode-1) end
    elseif game == 3 then
        enemyAddr = pointer + 0x352F4
        if mode == 5 then return pointer + 0x35AC4
        elseif mode == 4 then return memory.readdword(enemyAddr) + 0x7A0 + 0x5B0 + 0xEC*(submode-1)
        elseif mode == 3 then return memory.readdword(enemyAddr) + 0x7A0 + 0xB60 + 0xEC*(submode-1)
        elseif mode == 2 then return memory.readdword(enemyAddr) + 0x7A0 + 0xEC*(submode-1)
        else return pointer + 0xD094 + 0xEC*(submode-1) end
    elseif game == 4 then
        if mode == 5 then return 0x02259DD8
        elseif mode == 4 then return 0x0226B7B4 + 0xDC*(submode-1)
        elseif mode == 3 then return 0x0226C274 + 0xDC*(submode-1)
        elseif mode == 2 then return 0x0226ACF4 + 0xDC*(submode-1)
        else return 0x022349B4 + 0xDC*(submode-1) end
    elseif game == 5 then
        if mode == 5 then return 0x02259DF8
        elseif mode == 4 then return 0x0226B7D4 + 0xDC*(submode-1)
        elseif mode == 3 then return 0x0226C294 + 0xDC*(submode-1)
        elseif mode == 2 then return 0x0226AD14 + 0xDC*(submode-1)
        else return 0x022349D4 + 0xDC*(submode-1) end
    elseif game == 6 then
        if mode == 5 then return 0x0224795C
        elseif mode == 4 then return 0x022592F4 + 0xDC*(submode-1)
        elseif mode == 3 then return 0x02259DB4 + 0xDC*(submode-1)
        elseif mode == 2 then return 0x02258834 + 0xDC*(submode-1)
        else return 0x0221E3EC + 0xDC*(submode-1) end
    else
        if mode == 5 then return 0x0224799C
        elseif mode == 4 then return 0x02259334 + 0xDC*(submode-1)
        elseif mode == 3 then return 0x02259DF4 + 0xDC*(submode-1)
        elseif mode == 2 then return 0x02258874 + 0xDC*(submode-1)
        else return 0x0221E42C + 0xDC*(submode-1) end
    end
end

function getNatClr(a)
    color = "yellow"
    if nat % 6 == 0 then color = "yellow"
    elseif a == "atk" then
        if nat < 5 then color = "#0080FFFF" elseif nat % 5 == 0 then color = "red" end
    elseif a == "def" then
        if nat > 4 and nat < 10 then color = "#0080FFFF" elseif nat % 5 == 1 then color = "red" end
    elseif a == "spe" then
        if nat > 9 and nat < 15 then color = "#0080FFFF" elseif nat % 5 == 2 then color = "red" end
    elseif a == "spa" then
        if nat > 14 and nat < 20 then color = "#0080FFFF" elseif nat % 5 == 3 then color = "red" end
    elseif a == "spd" then
        if nat > 19 then color = "#0080FFFF" elseif nat % 5 == 4 then color = "red" end
    end
    return color
end
...
function read_pokemon_words(addr, num_words)
    ...
    if in_battle and num_bytes > 0x88 then
        ...
        for _, b in ipairs(battle_bytes) do bytes[#bytes + 1] = b end
    else
...
function check_gen5_in_battle(baseBattleAddr)
    ...
    expected_bytes = {0x62,0x74,0x6c,0x5f,0x70,0x6F,0x6B,0x65,0x70,0x61,0x72,0x61,0x6D,0x2E,0x63}
    for i = 1, 15 do if bytes[i] ~= expected_bytes[i] then return false end end
    return true
end

function hp_for_gen5_battles(words, q)
    local baseBattleAddr
    if game <= 3 or (words[1] == 0 and words[2] == 0) then return 0,0
    elseif game == 4 then baseBattleAddr = 0x0226D6B2
    elseif game == 5 then baseBattleAddr = 0x0226D6D2
    elseif game == 6 then baseBattleAddr = 0x0225B1F2
    else baseBattleAddr = 0x0225B232 end
    in_battle = check_gen5_in_battle(baseBattleAddr)
    if not in_battle then return nil end
    ...
    if (maxHp == 0 and currentHp == 0) or maxHp > 714 or currentHp > 714 or currentHp > maxHp then return 0,0 end
    in_battle = true
...
function do_print_first_pokemon_bytes(pidAddr)
    local byte_str = ""
    for _, b in ipairs(memory.readbyterange(pidAddr, 0x88)) do
...
function inspect_and_send_boxes()
    local box_offset = bills_pc_address[game] and bills_pc_address[game][subgame]
    if box_offset == nil then return true end
    ...
    if first_run then
        ...
        end_box_id = math.min(18, box_id_to_check + boxes_to_check_per_run - 1)
    end
    ...
    box_id_to_check = end_box_id + 1
    if box_id_to_check > 18 then box_id_to_check = 1 end
    return true
end
...
function kill_pokemon(pidAddr)
    ...
    if memory.readdword(pidAddr + 0x4E9F0) == pid then
        ...
        for i = 0, 3 do memory.writeword(pidAddr + 71 * 2 + (i * 0x400000), death_code) end
    end
end
...
function check_is_in_battle(addr, pid)
    local battle_addr = addr + 0x4E9F0
    for i = 0, 3 do
        if memory.readdword(battle_addr + (i * 0x400000), death_code) ~= pid then return false end
    end
    return true
end
...
function fn()
    ...
    if current_time - last_check > .5 then
        ...
        for q = 1, 6 do
            ...
            if game >= 4 then currentHp, maxHp = hp_for_gen5_battles(words, q) end
            ...
            if last_party == nil or last_party[q] == nil or in_battle or Pokemon.get_words_string(words) ~= last_party[q].data_str then
                ...
                if game >= 4 and in_battle and pokemon ~= nil then pokemon:update_hp(currentHp, maxHp) end
                party[q] = pokemon
            else
        ...
        if first_run then
            ...
            for k, pkmn in pairs(party) do
                last_party[k] = pkmn or Pokemon()
                send_data[#send_data + 1] = { slot_id = k, pokemon = pkmn }
            end
            print("================")
            print("DO NOT CLOSE THIS LUA SCRIPT WINDOW")
            print("You can minimize it, but do not close it, doing so will stop the emulator sending updates to SoulBuddy")
            print("================")
            first_run = false
        else
            for q = 1, 6 do
                ...
                lp = last_party[q]
                if p ~= nil then
                    if lp == nil or p ~= lp then send_data[#send_data + 1] = { slot_id = q, pokemon = p } end
                else
                    party[q] = lp
                end
            end
        ...
        if #send_data > 0 then send_slots(send_data, gen, game, subgame) end
